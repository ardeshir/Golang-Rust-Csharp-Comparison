...
fn main() -> Result<(), Box<dyn Error>> {  
    let matches = Command::new("Get URL")  
        .version("1.0")  
        .author("Rusty")  
        .about("Gets stuff")  
        .arg(  
            Arg::new("url")   
                .long("url")  
                .value_name("URL")  
                .help("Sets the URL to fetch"),
        )  
        .get_matches();
  // assert_eq!(m.get_one::<String>("foo").unwrap(), "foo");
    let link = "https://ardeshir.io/file.csv".to_string();
    let url = matches.get_one::<String>("url").unwrap_or(&link);  
